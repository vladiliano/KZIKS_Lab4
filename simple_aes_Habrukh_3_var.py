#!/usr/bin/env python3
#-*- coding: utf-8 -*-
# Source: https://jhafranco.com/2012/02/11/simplified-aes-implementation-in-python/
...
if __name__ == '__main__':
    ...
    plaintext = 0b1110110000110001  #EC31
    key = 0b0100000000001000        #4008
    ciphertext = 0b0111110010101011 #7CAB
    keyExp(key)
    try:
        assert encrypt(plaintext) == ciphertext
    except AssertionError:
        print("TEST 1: Encryption error")
    ...
    print("TEST 1 [ENC-DEC]: OK!")

    plaintext = 0b1000111111001111  #8FCF
    key = 0b0100000000001000        #4008
    ciphertext = 0b1001001001011010 #925A
    keyExp(key)
    try:
        assert decrypt(ciphertext) == plaintext
    except AssertionError:
    ...
    try:
        ...
        print(f"plaintext = {plaintext:016b} ({plaintext:04X}), key = {key:016b} ({key:04X}), ciphertext = {encrypt(plaintext):016b} ({encrypt(plaintext):04X})")
    print("TEST 1 [DEC-ENC]: OK!")

        #TEST2

    plaintext = 0b0110100000001010  #680A
    key = 0b1110001101100000        #E360
    ciphertext = 0b1101011010001011 #D68B
    keyExp(key)
    try:
        assert encrypt(plaintext) == ciphertext
    except AssertionError:
        print("TEST 2: Encryption error")
        print(encrypt(plaintext), ciphertext)
        sys.exit(1)
    else:
        print(f"plaintext = {plaintext:016b} ({plaintext:04X}), key = {key:016b} ({key:04X}), ciphertext = {encrypt(plaintext):016b} ({encrypt(plaintext):04X})")
    ...
    try:
        ...
        print("TEST 2: Decryption error")
        print(decrypt(ciphertext), plaintext)
        sys.exit(1)
    else:
        print(f"ciphertext = {ciphertext:016b} ({ciphertext:04X}), key = {key:016b} ({key:04X}), plaintext = {decrypt(ciphertext):016b} ({decrypt(ciphertext):04X})")
    print("TEST 2 [ENC-DEC]: OK!")

    plaintext = 0b110000010011010   #609A
    key = 0b1110001101100000        #E360
    ciphertext = 0b1101111010111011 #DEBB
    keyExp(key)
    try:
        assert decrypt(ciphertext) == plaintext
    except AssertionError:
